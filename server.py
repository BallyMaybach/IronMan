...
NOTION_TODO_PAGE_ID = config.get("notion_todo_page_id", "")
...
def get_notion_tasks_sync():
    """Fetch open to-dos from Notion, grouped by section heading."""
    if not NOTION_API_KEY or not NOTION_TODO_PAGE_ID:
        return []
    import urllib.request
    try:
        req = urllib.request.Request(
            f"https://api.notion.com/v1/blocks/{NOTION_TODO_PAGE_ID}/children?page_size=100",
            headers={
        ...
        for block in data.get("results", []):
            btype = block["type"]
            if btype == "heading_2":
                raw = "".join(t["plain_text"] for t in block["heading_2"]["rich_text"])
                # Strip emoji prefix (e.g. "🔴 Diese Woche" → "Diese Woche")
                current_section = raw.split(" ", 1)[-1] if " " in raw else raw
            elif btype == "to_do" and not block["to_do"]["checked"]:
                text = "".join(t["plain_text"] for t in block["to_do"]["rich_text"]).strip()
                if text:
                    sections.setdefault(current_section, []).append(text)
        # Flatten to list of "Abschnitt: task1, task2" strings
        result = []
        for section, tasks in sections.items():
            result.append(f"{section}: " + ", ".join(tasks))
        return result
    except Exception as e:
        print(f"[jarvis] Notion error: {e}", flush=True)
        return []


def refresh_data():
    global TASKS_INFO
    TASKS_INFO = get_notion_tasks_sync()
    print(f"[jarvis] Tasks: {len(TASKS_INFO)} Abschnitte geladen", flush=True)
...
WENN {USER_ADDRESS} "Jarvis activate" sagt:
- Begruesse ihn kurz passend zur Tageszeit (aktuelle Zeit: {{time}}). Ein Satz.
- Nenne danach die Aufgaben aus "Diese Woche" aus den AKTUELLE DATEN — fasse sie in einem Satz zusammen, nicht jede einzeln vorlesen. Falls keine vorhanden: kurz erwaehnen dass die Liste leer ist.
- Nichts ausser Begruessung + Todos. Kein "ich schaue mal", kein "lassen Sie mich", kein Smalltalk.
- ABSOLUTES VERBOT bei "Jarvis activate": KEINE Aktionen. Kein [ACTION:SCREEN], kein [ACTION:SEARCH], kein [ACTION:OPEN], nichts. Direkt Begruessung + Todos ausgeben, fertig.

=== AKTUELLE DATEN ==={task_block}
==="""
